...
const AllUsers = () => {
    ...
    const fetchAllUsers = async() => {
      try {
      axios
        .get('https://654b68155b38a59f28ef05c2.mockapi.io/scopex/api/users')
        .then(response => {
          setApiData(response.data);
          setIsLoading(false); // Set loading to false after data is fetched
        showToast({ type: "success", message: 'All Users Fetched successfully:'});
        })
        .catch(error => {
    ...
    const renderItem = ({ item }) => (
        <View style={styles.transactionItem}>
          ...
          <View style={styles.transactionDetails}>
            ...
            <Text style={styles.transactionText}>createdAt: {item.createdAt}</Text>
            <Text style={styles.transactionText}>kyc: {item.kyc ? 'Yes' : 'No'}</Text>
          </View>
        </View>
      );
